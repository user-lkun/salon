...
{//店铺活动控制器
    ...
    public  function  index(){
        ...
        if (!empty($_REQUEST['title'])) {
            $tiaojian[] = "title like '%{$_REQUEST['title']}%' ";
        }
...
//跳转首页
        $this->redirect("?p=Admin&c=Article&a=index", 0);
    }
    //添加热门活动
    public function add()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            if ($rs === false) {
                ...
                $this->redirect("?p=Admin&c=Article&a=add", '添加失败'.$this->model->getError(), 1);
            }
            //成功 调整到首页
            $this->redirect("?p=Admin&c=Article&a=index", '添加成功', 1);
    ...
    public  function  edit(){
        if ($_SERVER['REQUEST_METHOD']=="GET"){
            ...
            if ($c===false){
                $this->redirect("index.php?p=Admin&c=Article&a=edit&id={$data['id']}",'修改失败'.$this->model->getError(),2);
            }//修改成功跳转回首页
            $this->redirect('index.php?p=Admin&c=Article&a=index','修改成功',2);
    ...
    public  function  delete(){
        ...
        //删除完毕跳转回首页
        if ($result===false){
            $this->redirect('index.php?p=Admin&c=Article&a=index','删除失败'.$this->model->getError(),2);
        }//删除成功也跳转回首页
        $this->redirect('index.php?p=Admin&c=Article&a=index','',0);
    }

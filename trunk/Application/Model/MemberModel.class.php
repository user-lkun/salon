...
//判断密码不能空,并且密码不小于6位
    ...
    //>>1.准备sql
    $sql = "insert into `members`(username,password,telephone,last_login_time,photo,group_id,realname) VALUES (
'{$data['username']}',
'{$password}',
'{$data['telephone']}',
'{$last_login_time}',
...
//            $password = md5($data['newpassword']);
    ...
    {
        //根据用户名查询是否有服务记录,如果有,不能被删除
        $c=$this->db->fetchRow("select * from histories where members_id={$id}");
        if (!empty($c)){
            $this->error="不能删除有服务记录的员工";
            return false;
        }
...
//执行sql
  ...
  //查询所有的部门
    public  function  bumen(){
        return $this->db->fetchAll('select * from  `group`');
    }
}